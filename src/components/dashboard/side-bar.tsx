            ...
            <div className="flex w-full flex-col">
                <div className={`flex items-center h-16 ${!collapsed && 'px-4 gap-2'}`}>
                    <div className="flex items-center gap-3 justify-center">
                        {!collapsed && (
                            <Link to={'/dashboard'} className="text-start font-bold text-[#6A81FF]">
                                <img src={imagelogo} alt="logoimage" className="w-[200px]" />
                            </Link>
                        )}
                    </div>
                        ...
                        {navItems.map((n) => {
                            ...
                            return (
                                ...
                                >
                                    {!collapsed && (
                                        <span
                                            className={`w-[4px] h-[44px] block rounded-2xl  ${active ? "bg-[#6A81FF]" : ""
                                                }`}
                                        ></span>
                                    )}
                                    <NavLink
                                        key={n.href}
                                        to={n.href}
                                        className={`flex mx-auto text-[15px]  items-center gap-x-2 h-[44px] ${collapsed
                                    ...
                                    >
                                        {!collapsed ? (
                                            <n.icon className={`w-5 h-5 ${active ? "text-[#6A81FF]" : "text-[#666A7F] dark:text-[#e3dbdb]"}`} />
                                        ) : (
                                            <CustomTooltip tooltipText={t(n.labelKey)}>
                                                <n.icon className={`${active ? "text-[#6A81FF]" : "text-[#666A7F] dark:text-[#e3dbdb]"}`} />
                                            </CustomTooltip>
                                        )}
                                        {!collapsed && (
