...
const ProductCreate = () => {
	...
	const handleSalePriceUZS = (value: number) => {
		setData(prev => ({
			...
			saler_narxi: value,
			saler_narxi_dol:
				usdRate > 0 ? Math.round((value / usdRate) * 100) / 100 : 0,
		}))
	}
	...
	const handleUsdRateChange = (value: number) => {
		...
		setData(prev => ({
			...prev,
			tan_narx_dol:
				prev.tan_narx_uzb && value > 0
					? Math.round((prev.tan_narx_uzb / value) * 100) / 100
					: prev.tan_narx_dol || 0,
			saler_narxi_dol:
				prev.saler_narxi && value > 0
  ...
  [&::-webkit-scrollbar]:hidden bg-[#514e4e] text-white border rounded-md shadow-lg'
								...
								{/* Duplicate xabar */}
								{isDuplicate && (
									<p className='text-[12px] text-red-500 mt-1 absolute right-0 '>
										❌ Bu mahsulot mavjud
									</p>
								)}
							</div>
							...
							<label className='w-full flex flex-col space-y-2'>
								...
								<NumberInput
									...
									onChange={({ raw }) =>
										handleNumberChange('remine_count', raw)
									}
							...
							<label className='w-full flex flex-col space-y-2'>
								...
								<div className='relative'>
									<Input
										...
										onChange={e =>
											handleCostPriceUSD(parseFloat(e.target.value) || 0)
										}
							...
							<label className='w-full flex flex-col space-y-2'>
								...
								<div className='relative'>
									<Input
										...
										onChange={e =>
											handleSalePriceUSD(parseFloat(e.target.value) || 0)
										}
						...
						<div className='p-4 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg'>
							<p className='text-sm text-blue-800 dark:text-blue-200'>
								<strong>Eslatma:</strong> UZS va USD narxlari bir-biriga
								bog'langan. Qaysi birida yozsangiz ham, ikkinchisi avtomatik
								hisoblanadi.
								<br />
								Joriy kurs:{' '}
